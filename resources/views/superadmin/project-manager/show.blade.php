...
@section('content')
    <section class="container admin-list mt-3">
        <div class="d-flex mb-3">
            @include('partials._return', ['route' => 'project-manager.index'])
            <h1 class="mx-auto">View Project Manager</h1>
        </div>
        <div class="card mx-auto p-4" style="max-width: 600px; border-radius: 24px;">
            <div class="mb-4">
                <h2 class="fw-bold mb-4" style="font-size: 2rem;">{{ $projman['name'] }}</h2>
                <div class="mb-2">
                    <span class="tamad-baloo fw-bold">Email:</span>
                    <span class="tamad-nunito text-muted">{{ $projman['email'] ?? 'N/A' }}</span>
                </div>
                <div class="mb-2">
                    <span class="tamad-baloo fw-bold">Username:</span>
                    <span class="tamad-nunito text-muted">{{ $projman['username'] ?? 'N/A' }}</span>
                </div>
                <div class="mb-2 d-flex align-items-center">
                    ...
                    <input type="hidden" id="real-password" value="{{ $projman['password'] }}">
                ...
                <div class="d-flex flex-wrap gap-2">
                    @foreach ($projman['permissions'] as $permission)
